...
class SellerController extends IndexController {
    ...
    /********************************************卖家列表Ajax数据调用**************************************************/
    public function ajaxListSellers()
    {
        //定义模型
        $account=D('Account');
        ...
        if($data['_search']==false){
            //不是搜索的处理
            //获得总数据
            $ret['records']=$account->relation('Seller')->where(array('type'=>1,'is_passed'=>1))->count();//总条数
            $ret['total']=ceil($ret['records']/$pagesize);
            //获取当前页
            $page=$data['page'];
            $limit=$pagesize*($page-1).",".$pagesize;
            $ret['rows']=$account->relation('Seller')->where(array('type'=>1,'is_passed'=>1))->order(array('id','addate'=>'desc'))->field('id,account,password,type,is_passed,addate,add_p,add_s,add_q,add_d')->limit($limit)->select();
            foreach ($ret['rows'] as $key=>$value){
                $ret['rows'][$key]['dostr']="<a href='".U('sellerDel',array('id'=>$ret['rows'][$key]['id']))."' onclick='return del()'>删除</a>&nbsp;<a href='".U('editSeller',array('id'=>$ret['rows'][$key]['id']))."'>编辑</a>";
            }
            $ret['page']=$page;
            ...
            //组织条件str
            switch ($searc_oper){
                case eq:$map="type =1 and is_passed=1 and $searc_field = '".$searc_string."'";break;
                case ne:$map="type =1 and is_passed=1 and $searc_field != '".$searc_string."'";break;
                case lt:$map="type =1 and is_passed=1 and $searc_field < '".$searc_string."'";break;
                case le:$map="type =1 and is_passed=1 and $searc_field <= '".$searc_string."'";break;
                case gt:$map="type =1 and is_passed=1 and $searc_field > '".$searc_string."'";break;
                case ge:$map="type =1 and is_passed=1 and $searc_field >= '".$searc_string."'";break;
                default:$map="type =1 and is_passed=1";break;
            }
    ...
    /********************************************添加卖家处理**********************************************************/
    public function addSellerHanddle(){
    ...
    public function sellerDel(){
        ...
        $res2=M('seller')->where(array('aid'=>I('id')))->delete();
        if($res1&&$res2){
            M()->commit();
            $this->success('删除成功');
        }else{
    ...
    public function editSeller(){
        ...
        $seller=M('seller')->where(array('aid'=>I('id')))->find();
        //获取gps_x
    ...
    /********************************************编辑卖家处理**********************************************************/
    public function editSellerHanddle(){
        //判断是否为空
        ...
        //更改附表
        $res2=M('seller')->where(array('aid'=>I('id')))->save($data2);
