...
class CompanyController extends IndexController {
    ...
    public function ajaxList()
    {
        ...
        if($data['_search']==false){
            //不是搜索的处理
            //获得总数据
            $ret['records']=$account->relation('Company')->where(array('type'=>2,'is_passed'=>1))->count();//总条数
            $ret['total']=ceil($ret['records']/$pagesize);
            //获取当前页
            $page=$data['page'];
            $limit=$pagesize*($page-1).",".$pagesize;
            $ret['rows']=$account->relation('Company')->where(array('type'=>2,'is_passed'=>1))->order(array('id','addate'=>'desc'))->field('id,account,password,type,is_passed,addate,add_p,add_s,add_q,add_d')->limit($limit)->select();
            foreach ($ret['rows'] as $key=>$value){
                $ret['rows'][$key]['dostr']="<a href='".U('del',array('id'=>$ret['rows'][$key]['id']))."' onclick='return del()'>删除</a>&nbsp;<a href='".U('editCompany',array('id'=>$ret['rows'][$key]['id']))."'>编辑</a>";
            }
            $ret['page']=$page;
            ...
            //组织条件str
            switch ($searc_oper){
                case eq:$map="type =2 and is_passed=1 and $searc_field = '".$searc_string."'";break;
                case ne:$map="type =2 and is_passed=1 and $searc_field != '".$searc_string."'";break;
                case lt:$map="type =2 and is_passed=1 and $searc_field < '".$searc_string."'";break;
                case le:$map="type =2 and is_passed=1 and $searc_field <= '".$searc_string."'";break;
                case gt:$map="type =2 and is_passed=1 and $searc_field > '".$searc_string."'";break;
                case ge:$map="type =2 and is_passed=1 and $searc_field >= '".$searc_string."'";break;
                default:$map="type =2 and is_passed=1";break;
            }
    ...
    public function del(){
        ...
        $res2=M('company')->where(array('aid'=>I('id')))->delete();
        if($res1&&$res2){
            M()->commit();
            $this->success('删除成功');
        }else{
    ...
    public function editCompany(){
        ...
        $seller=M('company')->where(array('aid'=>I('id')))->find();
        //获取gps_x
    ...
    public function editHanddle(){
        ...
        //更改附表
        $res2=M('company')->where(array('aid'=>I('id')))->save($data2);
    ...
    public function ajaxListIspassed()
    {
        ...
        if($data['_search']==false){
            //不是搜索的处理
            //获得总数据
            $ret['records']=$account->relation('Buyer')->where(array('type'=>2,'is_passed'=>0))->count();//总条数
            $ret['total']=ceil($ret['records']/$pagesize);
            //获取当前页
            $page=$data['page'];
            $limit=$pagesize*($page-1).",".$pagesize;
            $ret['rows']=$account->relation('Company')->where(array('type'=>2,'is_passed'=>0))->order(array('id','addate'=>'desc'))->field('id,account,password,type,is_passed,addate,add_p,add_s,add_q,add_d')->limit($limit)->select();
            foreach ($ret['rows'] as $key=>$value){
                $ret['rows'][$key]['dostr']="<a href='".U('del',array('id'=>$ret['rows'][$key]['id']))."' onclick='return del()'>删除</a>&nbsp;<a href='".U('editCompany',array('id'=>$ret['rows'][$key]['id']))."'>编辑</a>&nbsp;<a href='".U('doPassed',array('id'=>$ret['rows'][$key]['id']))."'>审核</a>";
            }
            $ret['page']=$page;
            echo json_encode($ret);
        }else{
            ...
            //组织条件str
            switch ($searc_oper){
                case eq:$map="type =2 and is_passed=0 and $searc_field = '".$searc_string."'";break;
                case ne:$map="type =2 and is_passed=0 and $searc_field != '".$searc_string."'";break;
                case lt:$map="type =2 and is_passed=0 and $searc_field < '".$searc_string."'";break;
                case le:$map="type =2 and is_passed=0 and $searc_field <= '".$searc_string."'";break;
                case gt:$map="type =2 and is_passed=0 and $searc_field > '".$searc_string."'";break;
                case ge:$map="type =2 and is_passed=0 and $searc_field >= '".$searc_string."'";break;
                default:$map="type =2 and is_passed=0";break;
            }
            ...
            //获取当页数据
            $page=$data['page'];
            $ret['rows']=$account->relation('Company')->field('id,account,password,type,is_passed,addate,add_p,add_s,add_q,add_d')->order(array('id','addate'=>'desc'))->where($map)->select();
